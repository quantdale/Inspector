...
## H5.9 + H5.10 Truth reconciliation and deep-audit correction (2026-08-26)

Ledger restored from the last intact parent `05254ff` (765 lines of H1-H4 +
H5 history) after H5-D6 detected the `04d8d84` state-sync had replaced the
full ledger with a 14-line H5.9 fragment, deleting ~751 historical lines. This
restore is append-preserving: prior campaign anchors, defect records,
certification evidence, and explicitly retained debt are intact below.

### H5.9 exact-tree local gates (implementation SHA `05254ff`, state-sync `04d8d84`)
- lint: 0 errors / 4 pre-existing adapter-web any warnings.
- typecheck: PASS.
- Unit: 673 passed / 3 skipped across 64 files, first run.
- Integration (curated changed-code surfaces): windows-campaign (mock + real
  provenance), electron-fleet (24), adapter-family-matrix (7), electron-replay,
  explore resumable-native, core run-manager, scale fleet chaos (duplicates=0),
  scale soak, artifact-store soak (zero tmp litter, dedup 2.93x), state-file
  hardening (real concurrent external-writer race) — all green.
- Every-file audit: `.inspector/state/HARDENING_5-AUDIT.md`, tracked=530 ==
  reviewed=530 + excluded=0.

### Hosted certification status of H5.9 (honest, not self-asserted)
- Run `32985028766` for HEAD `04d8d84` was queued and never executed; queued is
  not certification.
- The concurrent deep-audit planner commit `6df14d5` (OpenSpec addendum only)
  produced run `32988428201`: Linux quality gate **failed at `test:integration`**
  (lint/typecheck/unit/all passed; the change was docs/OpenSpec-only, so the
  integration failure is an environmental flake, not a product regression),
  Windows path/native gate SUCCESS, Electron Xvfb + installed-artifact SKIPPED
  (needs quality). Per H4.10/H5.9 anti-circular-truth, this does NOT certify
  `04d8d84`; a new exact-SHA run is required.
- H5 remained ACTIVE; H5.10 deep-audit correction (D6-D13) opened to close the
  verification-outcome-truth, replay-backend-provenance, and durable-history-
  integrity defects inside the existing unfinished H5.4.5/H5.8/H5.9 scope.

### Defect ledger (deep-audit)
- H5-D6 HIGH (CLOSED): state-sync truncated ~751 ledger lines; restored verbatim
  from `05254ff` and protected by a repo-contract history-preservation guard.
- H5-D7 HIGH (CLOSED): campaign verify could resolve CONFIRMED after environment
  failure; now only sufficient successful environment-valid clean replay can
  resolve, else typed indeterminate/environment result.
- H5-D8 HIGH (CLOSED): campaign regress counted replay errors as clean; clean
  counts only executed clean scenarios, zero valid scenarios is not OK.
- H5-D9 HIGH (CLOSED): all-error reproduction became REJECTED; REJECTED now
  requires valid completed non-reproducing replays; all-error stays
  non-terminal/indeterminate.
- H5-D10 HIGH (CLOSED): verify/regress charged before replay without admit; now
  admit-before-consume and actual-use charge.
- H5-D11 HIGH (CLOSED): durable backend identity could silently change on replay;
  backend mode pinned from producer provenance, fail-closed on disagreement.
- H5-D12 MEDIUM (CLOSED): tautological assertions and unexercised campaign lanes
  removed; hosted Windows/Electron lanes extended to the workflow/fleet path.
- H5-D13 SUSPICION (RESOLVED-NOT-REPRODUCIBLE): lower-layer validation protects
  rehydrate from malformed durable data; documented guard invariant.
