# Repository-Local Add-ons Master Plan — Inspector

## Status

**PLANNING ONLY / IMPLEMENT IN A LATER SESSION**

Target repository: `quantdale/Inspector`  
Planning branch: `plan/repo-local-addons-2026-08-28`

## Repository assessment

Mature autonomous reliability platform with its own typed adapter protocol for Playwright, CLI/PTTY, Android/ADB, Windows/UIA and Electron. MCP is explicitly an external interoperability surface rather than its internal control plane.

**Decision:** `RECOMMEND_KNOWLEDGE_AND_CI_ONLY`

Inspector already has stronger native actuators than generic MCPs. The right additions improve knowledge and evidence visibility, not control.

## Recommended additions

### 1. Context7 MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Current documentation for Node/TypeScript and platform libraries without adding a competing actuator path.

**Constraints:** Docs only; Inspector's adapters remain the execution authority.

### 2. GitHub MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Useful for read-only exact-SHA Actions/status/code-security intelligence during hardening and release evidence review.

**Constraints:** Read-only, repo-focused profile. No issue/PR/release/branch writes and no runtime finding publication.

## Explicitly not recommended

- Chrome DevTools MCP as a replacement for Inspector's Playwright adapter
- Mobile MCP as a replacement for Inspector's ADB adapter
- Any MCP that becomes the internal adapter protocol
- Write-capable GitHub automation in the autonomous runtime
