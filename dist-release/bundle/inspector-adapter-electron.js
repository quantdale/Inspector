...
// packages/electron-adapter/src/electron-adapter.ts
...
var ArtifactStore = class {
  ...
  runDir(runId) {
    assertRunId(runId);
    return this.contain(join2(this.baseAbs, runId, "artifacts"));
  }
  /**
  ...
  ensureRunDirSafe(runId) {
    ...
    const runPath = join2(this.baseAbs, runId);
  ...
  write(options) {
    ...
    const absPath = this.contain(join2(dir, fileName));
  ...
  meta(runId, sha256) {
    ...
    const absPath = this.contain(join2(this.runDir(runId), sha256));
    let stat;
    try {
  ...
  clear() {
    if (dirname2(this.baseAbs) === this.baseAbs) {
...
var WebAdapterHandler = class {
  constructor(faults = {}, artifactBaseDir = join3(tmpdir(), "inspector-web-artifacts"), seedHtml, settleMs = 50, seedRedirectLoop = false, targetUrl) {
    this.faults = faults;
    ...
    this.artifactDir = mkdtempSync(join3(artifactBaseDir, "inst-"));
  ...
  async flushTrace() {
    ...
    const path = join3(this.artifactDir, `trace-${this.traceIndex++}.zip`);
    try {
...
var bin = resolveAdapterBin(import.meta.url, "inspector-adapter-web.js", "bin");

// packages/electron-adapter/src/electron-adapter.ts
var ELECTRON_CAPABILITIES = {
  protocolVersion: PROTOCOL_VERSION,
...
var ElectronAdapterHandler = class {
  web;
  mainLog = [];
  /** One-shot latch: the injected crash fault fires exactly once. */
  crashPending = false;
  seq = 0;
  constructor(faults = {}, artifactBaseDir = join4(tmpdir2(), "inspector-electron-artifacts"), seedHtml = SEED_HTML) {
  ...
  async observe(params = {}) {
    ...
    return {
      ...
      summary: {
        ...obs.summary,
        url: String(obs.summary.url ?? "").replace(/^http:/, "electron:"),
...
// packages/electron-adapter/src/bin.ts
var handler = new ElectronAdapterHandler();
...
//# sourceMappingURL=inspector-adapter-electron.js.map
